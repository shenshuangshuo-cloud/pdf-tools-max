...
App.register('sign', {
    ...
    processAndPreview() {
        ...
        // ── Adaptive local threshold (handles uneven lighting) ──────────────
        // Build integral image for O(1) box-mean queries
        ...
        const boxMean = (y1, x1, y2, x2) => {
            y1 = Math.max(0, y1); x1 = Math.max(0, x1);
            y2 = Math.min(h - 1, y2); x2 = Math.min(w - 1, x2);
            const area = (y2 - y1 + 1) * (x2 - x1 + 1);
            const a = integral[y2 * w + x2];
            const b = y1 > 0 ? integral[(y1 - 1) * w + x2] : 0;
            const c = x1 > 0 ? integral[y2 * w + (x1 - 1)] : 0;
            const d = (y1 > 0 && x1 > 0) ? integral[(y1 - 1) * w + (x1 - 1)] : 0;
            return (a - b - c + d) / area;
        };

        const radius = Math.max(6, Math.min(w, h) >> 5); // ~1/32 of smaller dimension
        const threshold = 12; // must be this much darker than local mean
        ...
        for (let y = 0; y < h; y++) {
            ...
            for (let x = 0; x < w; x++) {
                ...
                const localMean = boxMean(y - radius, x - radius, y + radius, x + radius);
                mask[rowBase + x] = (luma < localMean - threshold) ? 1 : 0;
            }
        }
        ...
        // ── Apply mask; preserve original color ─────────────────────────────
        for (let i = 0; i < data.length; i += 4) {
            data[i + 3] = cleaned[i >> 2] ? 255 : 0;
        }
    ...
    async renderPage(num) {
        ...
        const vp = page.getViewport({ scale: SIGN_PREVIEW_SCALE });
    ...
    async finalizeSign() {
        ...
        try {
            ...
            for (const sig of this.signatures) {
                ...
                const vp = pdfJsPage.getViewport({ scale: SIGN_PREVIEW_SCALE });
