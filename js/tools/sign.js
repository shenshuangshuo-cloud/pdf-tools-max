/* ========== Tool: Sign PDF (v6 - Better BG Removal + Crop/Rotate + Realistic Fonts) ========== */
...
App.register('sign', {
    ...
    // Upload state
    uploadOriginal: null,    // original Image element
    uploadRotation: 0,       // 0, 90, 180, 270
    uploadCrop: null,        // { x, y, w, h } in rotated image coords; null = no crop
    cropDragging: null,      // crop rectangle drag state
                    ...
                    <div id="sign-tab-upload" class="sign-tab-content" style="display:none">
                        ...
                        <div id="sign-upload-editor" style="display:none">
                            <div style="position:relative;display:inline-block;margin-top:10px;user-select:none" id="sign-upload-crop-container">
                                ...
                                <div id="sign-crop-overlay" style="display:none;position:absolute;inset:0;pointer-events:none">
                                    <div style="position:absolute;background:rgba(0,0,0,0.45)" id="crop-dark-top"></div>
                                    <div style="position:absolute;background:rgba(0,0,0,0.45)" id="crop-dark-bottom"></div>
                                    <div style="position:absolute;background:rgba(0,0,0,0.45)" id="crop-dark-left"></div>
                                    <div style="position:absolute;background:rgba(0,0,0,0.45)" id="crop-dark-right"></div>
                                    <div id="crop-border" style="position:absolute;border:2px dashed #fff;pointer-events:auto;cursor:move"></div>
                                </div>
                            </div>
                            <div style="display:flex;gap:10px;margin-top:10px;flex-wrap:wrap;align-items:center">
                                <button class="btn btn-sm" onclick="App.toolRegistry['sign'].rotateUpload(-90)">↺ Rotate Left</button>
                                <button class="btn btn-sm" onclick="App.toolRegistry['sign'].rotateUpload(90)">↻ Rotate Right</button>
                                <button class="btn btn-sm" id="sign-crop-btn" onclick="App.toolRegistry['sign'].toggleCrop()">✂ Crop</button>
                                <button class="btn btn-sm btn-primary" id="sign-crop-apply" style="display:none" onclick="App.toolRegistry['sign'].applyCrop()">Apply Crop</button>
                        ...
                        <div id="sign-upload-result" style="display:none;margin-top:12px;text-align:center">
                            <p style="font-size:0.8rem;color:var(--text-muted);margin:0 0 8px">Preview (background removed)</p>
    ...
    reset() {
        ...
        this.uploadOriginal = null; this.uploadRotation = 0; this.uploadCrop = null; this.cropDragging = null;
    },
    ...
    // ===================== SIGNATURE TAB: realistic handwritten fonts =====================
    renderStyleList() {
        ...
        const urls = this.generateRealisticSignatures(text);
        list.innerHTML = urls.map((url, i) => `
            <div class="sign-style-item${i === this.selectedStyle ? ' selected' : ''}" onclick="App.toolRegistry['sign'].selectStyle(${i})">
                ...
                <div class="sign-style-preview"><img src="${url}" style="max-height:52px"></div>
            </div>
        `).join('');
    },
    selectStyle(index) {
    ...
    generateRealisticSignatures(text) {
        const color = this.selectedColor;
        // 6 fonts with realistic handwriting profiles
        const profiles = [
            { font: '42px "Brush Script MT","Segoe Script","Apple Chancery",cursive', slant: -0.03, wobble: 5, spacing: 2 },
            { font: '40px "Segoe Script","Comic Sans MS","Snell Roundhand",cursive', slant: -0.01, wobble: 3, spacing: 1 },
            { font: '38px "Lucida Handwriting","Bradley Hand ITC","Zapfino",cursive', slant: -0.04, wobble: 6, spacing: 3 },
            { font: 'bold 36px "Freestyle Script","Mistral","Kunstler Script",cursive', slant: -0.05, wobble: 4, spacing: 1 },
            { font: 'italic 34px "Georgia","Times New Roman","Palatino",serif', slant: -0.02, wobble: 2, spacing: 2 },
            { font: '34px "Kristen ITC","Papyrus","Comic Sans MS",cursive', slant: -0.01, wobble: 3, spacing: 1 },
        ];
        return profiles.map(p => this.renderTextWithVariation(text, color, p));
    },

    renderTextWithVariation(text, color, profile) {
        const c = document.createElement('canvas');
        const padX = 30, padY = 20;
        c.width = text.length * 38 + padX * 2;
        c.height = 72;
        const ctx = c.getContext('2d');
        ctx.fillStyle = color;

        // Seed a deterministic random for this style so it's consistent
        let seed = text.split('').reduce((a, ch) => a + ch.charCodeAt(0), 0) + profile.font.length;

        // Draw each character with slight random rotation and vertical offset
        let x = padX;
        for (let i = 0; i < text.length; i++) {
            ...
            const angle = r1 * profile.slant * 2 + profile.slant * 2; // vary around base slant
        ...
        const actualW = x + padX - padX + 20;
        const finalCanvas = document.createElement('canvas');
        finalCanvas.width = actualW;
        finalCanvas.height = 72;
        finalCanvas.getContext('2d').drawImage(c, 0, 0, actualW, 72, 0, 0, actualW, 72);
    ...
    initDrawCanvas() {
        ...
        canvas.onmousedown = (e) => {
            self.isDrawing = true;
            const p = getPos(e);
            self.drawCtx.beginPath(); self.drawCtx.moveTo(p.x, p.y);
        };
        canvas.onmousemove = (e) => {
            if (!self.isDrawing) return;
            const p = getPos(e);
            self.drawCtx.lineTo(p.x, p.y); self.drawCtx.stroke();
        };
        canvas.onmouseup = () => { self.isDrawing = false; };
        canvas.onmouseleave = () => { self.isDrawing = false; };
        canvas.ontouchstart = (e) => {
            e.preventDefault(); self.isDrawing = true;
    ...
    renderUploadCanvas() {
        ...
        // Reset crop if image was re-rendered from rotation
        if (this.uploadCrop) {
            // Keep existing crop but re-render it
    ...
    rotateUpload(deg) {
        ...
        this.cancelCrop();
        this.renderUploadCanvas();
        this.processAndPreview();
    },

    // ===================== Crop =====================
    toggleCrop() {
        if (document.getElementById('sign-crop-overlay').style.display === 'none' || !document.getElementById('sign-crop-overlay').style.display) {
            this.startCrop();
        } else {
            this.cancelCrop();
        }
    ...
    startCrop() {
        ...
        // Default crop rect: center 60% of image
        ...
        border.style.width = cropW + 'px';
        border.style.height = cropH + 'px';
        ...
        border.onmousedown = function(e) {
            ...
            const onMove = (ev) => {
                ...
                const nl = Math.max(0, Math.min(cw - parseFloat(border.style.width), origLeft + dx));
                const nt = Math.max(0, Math.min(ch - parseFloat(border.style.height), origTop + dy));
                border.style.left = nl + 'px';
                border.style.top = nt + 'px';
                self.updateCropDark(nl, nt, parseFloat(border.style.width), parseFloat(border.style.height));
            };
    ...
    updateCropDark(x, y, w, h) {
        ...
        document.getElementById('crop-dark-bottom').style.cssText = `left:0;top:${y+h}px;width:100%;height:${ch-y-h}px`;
        document.getElementById('crop-dark-left').style.cssText = `left:0;top:${y}px;width:${x}px;height:${h}px`;
        document.getElementById('crop-dark-right').style.cssText = `left:${x+w}px;top:${y}px;width:${cw-x-w}px;height:${h}px`;
    },
    ...
    // ===================== Process: BG Removal (hard threshold) =====================
    processAndPreview() {
        const canvas = document.getElementById('sign-upload-canvas');
        ...
        // Hard threshold: pixel darker than 180 → keep as black, else fully transparent
        for (let i = 0; i < data.length; i += 4) {
            ...
            if (brightness > 175) {
                ...
                // Dark/signature → fully opaque black
                data[i] = 0;
                data[i + 1] = 0;
                data[i + 2] = 0;
                data[i + 3] = 255;
    ...
    applySetup() {
        ...
        if (activeTab === 'signature') {
            ...
            this.fullName = name;
            this.signatureImage = this.generateRealisticSignatures(name)[this.selectedStyle];
    ...
    onDragStart(e) {
        ...
        const onMove = (ev) => {
            ...
            overlay.style.width = d.origW + 'px'; overlay.style.height = d.origH + 'px';
        };
