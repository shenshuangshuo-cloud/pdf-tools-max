...
App.register('edit', {
    ...
    async renderPage(num) {
        ...
        // Force layout reflow: canvas.offsetWidth forces a synchronous layout.
        // On some browsers, reading offsetWidth once may still return 0 right after
        // display:none→display:block; a double read + rAF ensures the value is stable.
        let cw = canvas.offsetWidth;
        if (cw === 0) {
            // Yield a frame so the browser can complete layout
            await new Promise(r => requestAnimationFrame(r));
            // Force reflow
            cw = canvas.offsetHeight; // trigger layout
            cw = canvas.offsetWidth;
        }
        this._canvasScale = cw / canvas.width;
        // Guard against invalid scale (canvas not laid out, hidden, etc.)
    ...
    async _extractTextBlocks(page) {
        ...
        // Bug 1 fix: flip Y from PDF coordinates (bottom-left origin) to Canvas (top-left origin)
            ...
            for (const item of line) {
                if (!currentBlock) {
                    currentBlock = {
                        text: item.str,
                        x: item.x,
                        y: item.y - item.fontSize, // Bug 2: baseline→top in Canvas coords
                        fontSize: item.fontSize,
                        fontName: item.fontName,
                        rightX: item.x + (item.width || (() => {
                            // Bug 5: CJK-aware width estimation
                            let w = 0;
        ...
        const blocks = rawBlocks.map((rb, i) => ({
            ...
            color: rb.fontName.toLowerCase().includes('bold') ? '#333333' : '#000000', // Bug 6
        ...
        while (i < blocks.length) {
            ...
            let prevBlock = blocks[i]; // Bug 4: track previous line, not first line
            ...
            while (j < blocks.length) {
                ...
                if (Math.abs(next.x - prevBlock.x) < 15
                    ...
                    prevBlock = next; // Bug 4: advance to current line for next comparison
                    j++;
                } else {
                    break;
                }
            ...
            if (paraBlocks.length > 1) {
                ...
                mergedBlocks.push({
                    ...
                    color: first.bold ? '#333333' : '#000000', // Bug 6
                    bold: first.bold,
                    italic: first.italic,
                    combined: true,
                });
